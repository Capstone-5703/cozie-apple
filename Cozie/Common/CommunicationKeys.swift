...
public enum CommunicationKeys: String {
    ...
    case settingsTimestamp = "settings_timestamp"
...
// Shared by phone and watch: reject incomplete snapshots before changing stored settings.
enum SettingsSyncPayload {
    static func isValid(_ payload: [String: Any]) -> Bool {
        if payload[CommunicationKeys.settingsRevision.rawValue] != nil || payload[CommunicationKeys.settingsTimestamp.rawValue] != nil {
            guard let revision = payload[CommunicationKeys.settingsRevision.rawValue] as? String, !revision.isEmpty,
                  let timestamp = payload[CommunicationKeys.settingsTimestamp.rawValue] as? Double,
                  timestamp.isFinite, timestamp > 0 else { return false }
        }
        for key in [CommunicationKeys.userIDKey, .expIDKey, .passwordIDKey] {
            guard let value = payload[key.rawValue] as? String,
                  !value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return false }
        }
        guard payload[CommunicationKeys.writeApiURL.rawValue] is String,
              payload[CommunicationKeys.writeApiKey.rawValue] is String,
              payload[CommunicationKeys.userOneSignalIDKey.rawValue] is String,
              let interval = payload[CommunicationKeys.timeInterval.rawValue] as? Int, interval >= 0,
              let cutoff = payload[CommunicationKeys.healthCutoffTimeInterval.rawValue] as? Double,
              cutoff.isFinite, cutoff >= 0,
              let json = payload[CommunicationKeys.jsonKey.rawValue] as? Data,
              let survey = try? JSONDecoder().decode(WatchSurveyModelController.self, from: json),
              let firstID = survey.firstQuestionID,
              let first = survey.survey.first(where: { $0.questionID == firstID }),
              !first.responseOptions.isEmpty else { return false }
        return true
    }
}
